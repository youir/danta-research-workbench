...
矢量图源：[SVG 体系架构](00_系统_System/05_体系架构_Architecture.svg)。

从 Codex 对话提出任何研究任务，主引导会恢复上下文并选择需要的工作路径；遇到数据库查询、生物医学论文写作/修回或图稿任务时按需使用相应专项技能。论文写作可以直接说“帮我改摘要”“看看这条审稿意见”；助手先读取已授权的当前课题资料和稿件，不要求先写长提示词。Obsidian 负责浏览和整理同一个私有库。知识库不自动注入模型记忆；新会话时主引导按入口恢复必要内容。
