...
如果下载的是 `danta-research-vault-v1.8.0.zip`，直接解压并打开其中 `00_科研知识库_Research-Vault` 文件夹即可；已附主引导和角色配置，无需再运行初始化器。以下命令仅针对完整工作台发行包。
...
Codex：打开同一新库，发“先读 AGENTS.md 和 00_index.md，按 Path-Map 恢复状态与 Idea 画布，了解我的资料，再帮我推进当前问题。”知识库不是自动注入模型的永久记忆；这套读取协议让新会话有依据地恢复。
