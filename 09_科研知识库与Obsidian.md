...
想让 Codex 从既有文章和课题材料中了解研究脉络时，可启用初始资料扫描。它必须先向龚博士说明范围并取得本人明确授权；扫描只读，重点摘要附来源供本人核对。vault 的“研究背景”区附有报告模板。
...
> 使用 $danta-proposal-guide。我是龚博士，请按这个知识库的索引恢复必要上下文。

配置完成后，龚博士可以直接说“改摘要”“帮我看这条审稿意见”“准备组会”或用自己的话提出需求，不必每次复制启动语、介绍课题或写长提示词。主引导会按任务读取索引、研究画像、当前状态、稿件入口和相关来源；只有关键材料确实找不到或不同稿件互相冲突时才简短询问。新使用者不先安装第三方 skills、agent 插件或科学计算环境；任务需要时再单独配置。
