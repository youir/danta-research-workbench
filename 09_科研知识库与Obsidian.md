...
下载本版 `danta-research-vault-v1.10.0.zip`，解压到她自己的私有研究目录，然后在 Obsidian 与 Codex 打开同一个 `00_科研知识库_Research-Vault` 文件夹，再打开 `00_index.md`。主引导和角色配置已附带，不含真实研究资料，也不要求先安装其他技能或执行终端命令。
