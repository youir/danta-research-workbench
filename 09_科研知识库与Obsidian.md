...
这是 v1.7 新增的完整空白科研知识库：从 00 开始的中英文目录、每层 `00_index.md`、核心 Templates 笔记模板、来源追溯、OGSM 与长期记忆。参考用户指定 hwm 的索引、raw→wiki→output 与低噪音记忆原则重新编写，不包含其个人或业务资料。

## 最省事：下载可直接打开的知识库ZIP

下载本版 `danta-research-vault-v1.9.0.zip`，解压后在 Obsidian 和 Codex 打开同一个 `00_科研知识库_Research-Vault` 文件夹，再打开 `00_index.md`。此ZIP由初始化器从空白模板生成，不含真实研究数据；不需要再次执行初始化。
...
> 先读 AGENTS.md、00_index.md 和 00_系统_System/03_路径映射_Path-Map.json，按映射恢复当前状态与Idea画布。先理解我的资料与资源，再帮助我思考。按需读取知识库，区分来源、推断与我的决定；结束时只保存值得长期使用的记忆和未闭环事项。
