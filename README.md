...
本仓库是可下载的科研辅助工作区：包含龚博士的主引导 skill、固定版本科研 skills、生物数据库查询、生物医学论文写作与修回、生物科研日报、PPT 与科研图工作流、PPT Master 官方固定版本安装器、女娲专家蒸馏 skill、可选专家角色配置和空白模板。它需要支持文件与工具的 AI 客户端运行，不是独立网页应用。PPT Master 不占用日常知识库空间，第一次需要真正制图时再按校验值下载到当前私有工作区。

## 最快开始：下载一个知识库，打开两个应用

龚博士已经使用 Obsidian，并已有 `GY` 知识库。建议先让 Codex 在龚博士自己的电脑上确认 Obsidian 当前打开的 vault 是否为 `GY`，再把插件安装到这个现有库；不另建 vault，也不要求迁移旧笔记。下载 [独立知识库 ZIP](https://github.com/youir/danta-research-workbench/releases/latest/download/danta-research-vault-v1.18.1.zip) 主要用于全新使用者。主引导、思维引导、生物数据库与生物医学论文写作、生物科研日报专项技能、PPT/科研图流程、RSS 信息源、管理面板与空白模板都已附带，无须先安装整套技能。第一次要求真正制作 PPT 或科研图时，Codex 才下载安装并配置所需工具。请复制 [本机部署与首次配置](12_本机部署与首次配置.md) 的启动语。
...
![龚博士科研工作台最新框架图：一个对话入口、思考与持续研究、组会汇报和带证据状态的可编辑科研图，共用唯一私有研究档案](docs/assets/architecture.png)
...
- [生物研究安全审查 SOP](15_生物研究安全审查与工作连续性.md)：审查触发后保留脱敏上下文、继续允许的范围，并核验官方可信访问条件；不自动向其他模型转交受审查内容。
...
- [科研汇报与可编辑PPT指南](13_科研汇报与可编辑PPT指南.md)：组会/开题PPT、机制图、流程图、实验设计图与病理图版
- [验证与优化说明](04_打包验证说明.md)

本仓库不含真实研究数据、私人聊天、账号配置或已启用的后台日报。科研日报按需生成；其线索需回到原始来源核实，复制工作区不会迁移已有自动化。

## v1.12 PPT Master 与科研图

增加 PPT Master v6.6.0 官方压缩包 SHA-256 校验安装器，第一次制图时才下载到当前私有知识库，保留其完整性/归属检查和上游文件，不把 62 MB 的包塞进日常库。PPT 与独立科研图统一走主引导，支持机制图、流程/实验设计图、病理图版和真实数据图；增加机制关系来源表、图例语义、模型语境、已完成/计划状态、真实图像与定量数据边界，以及科研图索引和模板。默认禁用上游在线研究、搜图和生成式配图。
...
新增按需触发的生物科研日报技能：参考 [kawaiier/daily-brief](https://github.com/kawaiier/daily-brief) 将来源收集、结构化内容与 HTML 呈现分开，并采用短清单、不凑数、来源可追溯和明确空状态。使用已有 RSS/Atom 信息源，不假设日历、邮件或聊天已连接；不订阅新源、不开启更新或创建定时任务。日报可附一条组会讨论问题，但只从实际文献线索提出，不代写未提供的会议事项。输出为私有知识库中的每日独立 HTML 文件。

## v1.16 生物医学论文写作与修回

升级 `danta-bio-paper-writing` 为龚博士的轻量论文协作者，覆盖中英文起草/修改、摘要与章节、图注、全文逻辑检查、审稿意见回复和投稿材料。她可以用“帮我改摘要”或“看看这条审稿意见”这类短指令启动；助手先从已授权的 GY 知识库、当前稿件和已确认的偏好恢复相关上下文，只追问会改变科学判断的缺项。简单编辑直接交付，长稿才先对齐论证提纲和证据缺口。

吸收本机生信技能库中“结果问题—证据—图表映射”、方法复现要素和报告规范缺项分类；不沿用固定服务菜单、默认整套分析/制图、长篇 PRD 审批或特定 Codar 服务器导出依赖。写作不自动触发数据分析、扫描未授权目录或科研图生成；这些仍由对应工作流按需处理。论文写作偏好模板留空，待龚博士在实际协作中逐步确认。

## v1.17 生物研究安全审查后的工作连续性

如果某项生物研究请求触发安全审查，主引导会暂停对应部分，在授权的私有知识库记录最少且脱敏的工作状态，并继续可分离、获准的病理背景、公开文献、写作、合规或归档任务。若有官方研究人员可信访问流程，可以协助核验条件与准备真实申请材料。安全审查不会自动启动 KimiWork、其他模型或文件上传来续做同一受审查请求；独立安全任务的外部服务使用须单独说明范围和资料处理并取得明确同意。
...
v1.11 提供组会/开题/阶段汇报工作流。v1.12 将制图引擎更新为 PPT Master；v1.11 的 `presentation-skill` 作为历史发行依赖，不再是当前推荐引擎。
